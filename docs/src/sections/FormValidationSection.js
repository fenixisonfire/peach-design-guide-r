...
export default function FormValidationSection() {
  return (
    <div className="bs-docs-section">
      ...
      <p>Set <code>validationState</code> to one of <code>'success'</code>, <code>'warning'</code> or <code>'error'</code> to show validation state. Set <code>validationState</code> to <code>null</code> (or <code>undefined</code>) to hide validation state. Add <code>{'<FormControl.Feedback>'}</code> for a feedback icon when validation state is set. The following examples follow the intuitive PEACH colour scheme.</p>
